...
def render_dashboard_page(sales_db: str):
    ...
    if col_date:
        temp_df["parsed_invoice_date"] = pd.to_datetime(
            temp_df[col_date], format="%d/%m/%Y", errors="coerce"
        )
        if temp_df["parsed_invoice_date"].isna().all():
            temp_df["parsed_invoice_date"] = pd.to_datetime(
                temp_df[col_date], errors="coerce"
            )

    curr_df = temp_df.copy()

    # Row 1: Primary Categorical Filters
    ...
    # Row 2: Date Range, Invoice No & Actions
