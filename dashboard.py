...
def update_gsheet_by_id(sheet_name: str, id_col: str, modified_df: pd.DataFrame) -> bool:
    try:
        ...
        if id_col_clean not in headers_lookup:
            st.error(f"Column '{id_col}' not found in Google Sheet.")
            return False
        ...
        for _, row in modified_df.iterrows():
            ...
            if not target_sheet_row:
                st.warning(f"⚠️ Row for ID #{target_id} not found in Google Sheet.")
                continue
...
def render_dashboard_page(sales_db: str):
    ...
    DEFAULT_COLUMNS = [
        ...
        "cgst", "sgst", "invoiced_value_fc", "challan_no", "vehicle_no",
        "vehicle_type", "eway_bill_no", "approx_distance", "provisional_freight_amount",
...
#         if new_cols_added:
#             # Update header row in single request
#             worksheet.update("1:1", [raw_headers])
...
#     #     )
#     #     if col_id not in selected_display_cols:
#     #         selected_display_cols.insert(0, col_id)

#     # with c2:
#     #     new_col_input = st.text_input("Add New Column Name:")
#     #     if st.button("➕ Add Column") and new_col_input:
#     #         clean_name = new_col_input.strip()
#     #         if clean_name and clean_name not in st.session_state.working_df.columns:
#     #             st.session_state.working_df[clean_name] = ""
#     #             st.success(f"Added column '{clean_name}'")
#     #             st.rerun()
...
#     with top_col3:
#         st.markdown("<br>", unsafe_allow_html=True)
#         # export_df = filtered_df[DEFAULT_COLUMNS]
#         export_df = filtered_df
#         excel_buffer = io.BytesIO()
#         with pd.ExcelWriter(excel_buffer, engine="openpyxl") as writer:
#             export_df.to_excel(writer, index=False, sheet_name="Logistics Data")
...
#     # Prepare current page slice safely
#     display_df = filtered_df[DEFAULT_COLUMNS]
#     page_data = display_df.iloc[start_idx:end_idx].copy()


#     # 1. Start by setting default configuration for ALL columns to disabled=True
#     column_configuration = {
#         col: st.column_config.Column(
#             label=col.replace("_", " ").title(), # Format column names cleanly (e.g., location_name -> Location Name)
#             disabled=True
#         )
#         for col in display_df.columns
#     }

#     # 2. Override specific columns that SHOULD be editable or need custom formatting
#     column_configuration.update({
#         # Strictly Disabled with Special Formatting
#         col_id: st.column_config.Column("ID", disabled=True),
#         "provisional_perc": st.column_config.NumberColumn("Provisional %", disabled=True, format="%.2f%%"),
#         "actual_perc": st.column_config.NumberColumn("Actual %", disabled=True, format="%.2f%%"),
...
#     # Save edited changes from Streamlit data editor into state persist memory using 'id'
#     editor_state = st.session_state.get("data_editor_grid", {})
#     grid_edited_rows = editor_state.get("edited_rows", {})

#     if grid_edited_rows:
#         for row_idx, changes in grid_edited_rows.items():
#             if row_idx < len(page_data):
#                 record_id = str(page_data.iloc[row_idx][col_id]).strip()
#                 if record_id not in st.session_state.pending_edits:
#                     st.session_state.pending_edits[record_id] = {}
#                 st.session_state.pending_edits[record_id].update(changes)

#                 # Update working dataframe state immediately
#                 main_mask = st.session_state.working_df[col_id].astype(str).str.strip() == record_id
#                 for k, v in changes.items():
#                     st.session_state.working_df.loc[main_mask, k] = v

#         # Recalculate freight percentages
#         st.session_state.working_df = calculate_freight_percentages(st.session_state.working_df)

#     # Persistent Save Button (Stays visible as long as pending edits exist)
#     if st.session_state.pending_edits:
#         st.info(f"📝 You have unsaved changes for **{len(st.session_state.pending_edits)}** record(s).")
#         if st.button("💾 Save Changes to Google Sheet", type="primary"):
#             modified_ids = list(st.session_state.pending_edits.keys())
...
# # It works good but when we enter a value in provisional or actual freight amount then immediately should display the its corresponding percentage after any event.
...
# def update_gsheet_by_id(sheet_name: str, id_col: str, modified_df: pd.DataFrame) -> bool:
#     """
#     Updates Google Sheet rows safely by using batch fetches and batch updates.
#     Uses the unique 'id' column as the primary key.
#     """
#     try:
#         spreadsheet = connect_gsheet()
#         worksheet = spreadsheet.worksheet(sheet_name)
...
# def gspread_cell_format(row: int, col: int) -> str:
#     """Converts 1-based row and col index to A1 notation (e.g. row 2, col 3 -> C2)."""
#     col_str = ""
#     while col > 0:
#         col, remainder = divmod(col - 1, 26)
#         col_str = chr(65 + remainder) + col_str
...
#     # ─────────────────────────────────────────────────────────────────
#     #  CRITICAL FIX: PROCESS EDITS & RECALCULATE BEFORE SCLICING DATA
#     # ─────────────────────────────────────────────────────────────────
#     editor_state = st.session_state.get("data_editor_grid", {})
#     grid_edited_rows = editor_state.get("edited_rows", {})

#     # Slice temporary slice to match exact row index mapping
#     pre_slice = filtered_df[DEFAULT_COLUMNS].iloc[start_idx:end_idx]

#     if grid_edited_rows:
#         has_new_changes = False
#         for row_idx, changes in grid_edited_rows.items():
#             if int(row_idx) < len(pre_slice):
#                 record_id = str(pre_slice.iloc[int(row_idx)][col_id]).strip()
#                 if record_id not in st.session_state.pending_edits:
#                     st.session_state.pending_edits[record_id] = {}
...
#         if has_new_changes:
#             # Immediately recalculate freight percentages
#             st.session_state.working_df = calculate_freight_percentages(st.session_state.working_df)
            
#             # Update filtered_df so page data gets freshly calculated figures
#             filtered_df.update(st.session_state.working_df)
#             st.rerun()

#     # Prepare page slice with UPDATED values & percentages
#     display_df = filtered_df[DEFAULT_COLUMNS]
#     page_data = display_df.iloc[start_idx:end_idx].copy()
...
# # It takes more time to save the data and calculate the percentage? and multiple users will working on this application, so take care of data should be saved correctly , for that if need to use cache please use it. and try to make it faster.
